...
class ReferralCreateICD9Screen extends Component {
  ...
  _icd9() {
    ...
    HealthcareAPI.get("/diagnosis/icd/list", {
      ...
      this.setState({ icd9: response.data, spinner: false })
    );
  }
  ...
  _updateSelectedICD9(icd9) {
    ...
    if (
      current.filter((item) => item.idDiagnosis === icd9.idDiagnosis).length > 0
    ) {
  ...
  _renderICD9(item) {
    let isSelected =
      this.state.selectedICD9.filter(
        (icd9) => icd9.idDiagnosis === item.idDiagnosis
      ).length > 0;
    return (
      <TouchableOpacity
  ...
  _setICD9Screen() {
    return (
      <FlatList
        data={this.state.icd9.filter((icd) =>
          icd.namaDiagnosis
            .toLowerCase()
  ...
  _setSelectedICD9Screen() {
    return this.state.selectedICD9.map((icd9) => {
      return (
        <TouchableOpacity
          onPress={() => this._updateSelectedICD9(icd9)}
          key={icd9.idDiagnosis}
            ...
            <TouchableOpacity
              onPress={() =>
                navigate("ReferralCreateICD10Screen", {
                  ...
                  docPemeriksaanDarah: this.props.navigation.getParam(
                    "docPemeriksaanDarah"
                  ),
                  docPemeriksaanLain: this.props.navigation.getParam(
                    "docPemeriksaanLain"
                  ),
