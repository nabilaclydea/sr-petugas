...
class ReferralCreateScheduleDestinationScreen extends Component {
  ...
  componentWillMount() {
    AsyncStorage.getItem("user").then((result) => {
      ...
      tmp.rujukan.tanggalPasienDirujuk =
        today.getFullYear() +
        "-" +
        this._doubleDigit(today.getMonth() + 1) +
        "-" +
        this._doubleDigit(today.getDate());
  ...
  _cityOfHealthFacility(id) {
    HealthcareAPI.get("/city/health/facility", { params: { id: id } }).then(
      (response) => {
  ...
  _healthFacilityByCity(id) {
    HealthcareAPI.get("/health/facility/city", { params: { idKota: id } }).then(
      (response) => {
  ...
  _healthFacilityVertical(id) {
    HealthcareAPI.get("/health/facility/vertical", { params: { id: id } }).then(
      (response) => {
        let list = response.data;
        let tmp = this.state.referralForm;
        tmp.rujukan.idFaskesTujuan = list[0].idFaskes;
  ...
  _polis() {
    HealthcareAPI.get("/poli/type/list").then((response) => {
      let list = response.data;
      let tmp = this.state.referralForm;
  ...
  _poliOfHealthFacility(id) {
    HealthcareAPI.get("/poli/health/facility/list", {
      params: { id: id },
    }).then((response) => {
      let list = response.data;
      let tmp = this.state.referralForm;
  ...
  _bloodType() {
    HealthcareAPI.get("/blood/type/list").then((response) => {
      let list = response.data;
      let tmp = this.state.referralForm;
            ...
            <View style={{ margin: 5 + "%" }}>
              ...
              <View style={{ marginTop: 15 }}>
                ...
                >
                  <Picker
                    selectedValue={
                      this.state.referralForm.rujukan.idFaskesTujuan
                    }
                    onValueChange={(itemValue, itemIndex) => {
                      let tmp = this.state.referralForm;
                  ...
                  >
                    {this.state.healthFacilities.map((healthFacility) => (
                      <Picker.Item
                        label={healthFacility.nama}
                        value={healthFacility.idFaskes}
                        key={healthFacility.idFaskes}
                      />
                    ))}
                  </Picker>
                </View>
                ...
                >
                  ...
                  >
                    {this.state.polis.map((poli) => (
                      <Picker.Item
                        label={poli.namaJenisPoli}
                        value={poli.idJenisPoli}
                        key={poli.idJenisPoli}
                      />
                    ))}
                  </Picker>
                </View>
              ...
              <View style={{ marginTop: 15 }}>
                ...
                >
                  <TouchableOpacity onPress={this._showDateTimePicker}>
                    <TextInput
                      value={
                        this.state.referralDate.getDate() +
                        " / " +
                ...
                <DateTimePicker
                  ...
                  onConfirm={(date) => {
                    ...
                    tmp2.rujukan.tanggalPasienDirujuk =
                      tmp.getFullYear() +
                      "-" +
      ...
      return (
        <View>
            ...
            <View style={{ margin: 5 + "%" }}>
              <View>
                ...
                >
                  <Picker
                    selectedValue={
                      this.state.referralForm.rujukan.idTipeRuangan
                    }
                    onValueChange={(itemValue, itemIndex) => {
                      let tmp = this.state.referralForm;
              ...
              <View style={{ marginTop: 15 }}>
                ...
                >
                  <Picker
                    selectedValue={
                      this.state.referralForm.pasien.idGolonganDarah
                    }
                    onValueChange={(itemValue, itemIndex) => {
                      let tmp = this.state.referralForm;
                  ...
                  >
                    {this.state.bloods.map((blood) => (
                      <Picker.Item
                        label={blood.namaGolonganDarah}
                        value={blood.idGolonganDarah}
                        key={blood.idGolonganDarah}
                      />
                    ))}
                  </Picker>
                </View>
                  ...
                  <View styl={{ marginLeft: 4 }}>
                    <MultiSelect
                      ...
                      onSelectedItemsChange={(selectedItems) =>
                        this._onSelectedItemsChange(selectedItems)
                      }
          ...
          <View style={{ alignItems: "center", width: 100 + "%", height: 55 }}>
            <TouchableOpacity
              onPress={() => this._searchHealthFacility()}
              style={{
                justifyContent: "center",
                alignItems: "center",
                width: 80 + "%",
                ...
                <View key={healthFacility.idFaskes}>
                  ...
                  >
                    <View style={{ flexDirection: "column", width: 70 + "%" }}>
                      ...
                      <Text style={{ color: "#cacaca", fontSize: 10 }}>
                        Kamar Kosong:
                      </Text>
                      <Text
                        style={{
                      ...
                      >
                        {healthFacility.jumlahBedTersedia}
                      </Text>
                      <Text style={{ color: "#cacaca", fontSize: 10 }}>
                        Fasilitas Tersedia:
                      </Text>
  ...
  _searchHealthFacility() {
    ...
    HealthcareAPI.get("/health/facility/room", {
      params: { idTipeRuangan: this.state.selectedRoom },
    }).then((response) => {
  ...
  render() {
    return (
      <View style={styles.firstLayer}>
        ...
        <ScrollView>{this._setScreen()}</ScrollView>
      </View>
    );
  }
}
